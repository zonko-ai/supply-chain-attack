...
function scanSuspiciousNpmScripts(packages) {
  ...
  for (const item of packages) {
    ...
    const evidenceFiles = containsCurlCommand(command) ? [] : postinstallFilesWithCurl(command, item.packageRoot);
    if (!containsCurlCommand(command) && !evidenceFiles.length) continue;

    const key = `${packageKey(item.ecosystem, item.name, item.version)}:${command}:${evidenceFiles.join(',')}`;
    if (seen.has(key)) continue;
    seen.add(key);
    ...
    findings.push({
      ...
      locations: item.locations || [item.location].filter(Boolean),
      sources: item.sources || [item.source].filter(Boolean),
      reason: evidenceFiles.length ? 'postinstall script references a local file that invokes curl' : 'postinstall script invokes curl',
    });
  }
...
function postinstallFilesWithCurl(command, packageRoot) {
  ...
  for (const candidate of postinstallFileCandidates(command)) {
    ...
    if (containsCurlCommand(safeReadText(filePath, MAX_MANIFEST_BYTES))) {
      result.push(displayPath(filePath));
    }
  }
...
function containsCurlCommand(command) {
  return /(?:^|[^A-Za-z0-9_-])curl(?:$|[^A-Za-z0-9_-])/i.test(command);
}
