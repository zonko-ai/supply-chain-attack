...
async function scanMachine(options = {}) {
  ...
  const suspiciousScripts = scanSuspiciousNpmScripts(packages);
  ...
  for (const indexRoot of indexRoots) {
    for (const shard of safeReaddirEntries(indexRoot)) {
      ...
      for (const entry of safeReaddirEntries(shardPath)) {
        ...
        if (parsed) {
          const source = displayPath(path.join(shardPath, entry.name));
          const indexInfo = readJson(path.join(shardPath, entry.name));
          packages.push(machinePackage('npm', parsed.name, parsed.version, label, source, {
            packageJson: indexInfo && typeof indexInfo === 'object' ? indexInfo : undefined,
          }));
        }
      }
    }
...
function scanSuspiciousNpmScripts(packages) {
  const findings = [];
  const seen = new Set();

  for (const item of packages) {
    if (item.ecosystem !== 'npm' || !item.packageJson || typeof item.packageJson !== 'object') continue;
    ...
    const commandIndicators = networkFetchIndicators(command);
    const evidenceFiles = commandIndicators.length ? [] : postinstallFilesWithNetworkFetch(command, item.packageRoot);
    if (!commandIndicators.length && !evidenceFiles.length) continue;

    const key = `${packageKey(item.ecosystem, item.name, item.version)}:${command}:${evidenceFiles.map((item) => item.path).join(',')}`;
    if (seen.has(key)) continue;
    seen.add(key);
    ...
    findings.push({
      ...
      script: 'postinstall',
      command,
      evidenceFiles: evidenceFiles.map((item) => item.path),
      indicators: commandIndicators.length ? commandIndicators : Array.from(new Set(evidenceFiles.flatMap((item) => item.indicators))),
      locations: item.locations || [item.location].filter(Boolean),
      sources: item.sources || [item.source].filter(Boolean),
      reason: evidenceFiles.length ? 'postinstall script references a local file with network-fetch behavior' : 'postinstall script has network-fetch behavior',
    });
  }

  findings.sort((a, b) => `${a.name}@${a.version}`.localeCompare(`${b.name}@${b.version}`));
  return findings;
}

function postinstallFilesWithNetworkFetch(command, packageRoot) {
  if (!packageRoot) return [];

  const result = [];
  for (const candidate of postinstallFileCandidates(command)) {
    const filePath = path.resolve(packageRoot, candidate);
    if (!isPathInside(filePath, packageRoot)) continue;

    const stat = safeStat(filePath);
    if (!stat.isFile() || stat.size <= 0 || stat.size > MAX_MANIFEST_BYTES) continue;

    const indicators = networkFetchIndicators(safeReadText(filePath, MAX_MANIFEST_BYTES));
    if (indicators.length) {
      result.push({ path: displayPath(filePath), indicators });
    }
  }
  ...
  for (const item of result) {
    ...
    seen.set(item.path, Array.from(new Set([...existing, ...item.indicators])).sort());
  }
  return Array.from(seen, ([filePath, indicators]) => ({ path: filePath, indicators }))
    .sort((a, b) => a.path.localeCompare(b.path));
}

function postinstallFileCandidates(command) {
  const candidates = [];
  ...
  while ((match = tokenPattern.exec(command))) {
    ...
    if (/^(?:node|sh|bash|zsh|dash|env|npm|pnpm|yarn|bun|run|exec|install|postinstall)$/i.test(token)) continue;
    if (/^[A-Za-z_][A-Za-z0-9_]*=/.test(token)) continue;

    const normalized = token.replace(/^\.\//, '');
    if (/^(?:\.\.?\/|\/)/.test(token) || token.includes('/') || /\.(?:js|cjs|mjs|sh|bash|zsh|cmd|ps1)$/i.test(token)) {
...
function networkFetchIndicators(text) {
  const indicators = [];
  if (/(?:^|[^A-Za-z0-9_-])curl(?:$|[^A-Za-z0-9_-])/i.test(text)) indicators.push('curl');
  if (/(?:^|[^A-Za-z0-9_-])wget(?:$|[^A-Za-z0-9_-])/i.test(text)) indicators.push('wget');
  if (/\bfetch\s*\(/i.test(text)) indicators.push('fetch');
  if (/\b(?:http|https)\.(?:get|request)\s*\(/i.test(text)) indicators.push('http-request');
  if (/\bnode:(?:http|https)\b|\brequire\s*\(\s*['"](?:http|https)['"]\s*\)/i.test(text)) indicators.push('http-module');
  return indicators;
}
