...
async function runInteractive(result, options = {}) {
  ...
  try {
    ...
    process.stdout.write(formatMenu());
    while (true) {
      const answer = (await rl.question('amifcked> ')).trim().toLowerCase();
      if (!answer || answer === 'q' || answer === 'quit' || answer === 'exit') {
        process.stdout.write('cool. stay paranoid.\n');
        break;
      }

      if (answer === '1' || answer === 'e' || answer.includes('educate')) {
        process.stdout.write(`\n${formatEducation(result, options)}\n`);
        process.stdout.write(formatMenu({ compact: true }));
        continue;
      }
      ...
      process.stdout.write('pick 1, 2, or q. I am not a mind reader.\n');
    }
  } finally {
    rl.close();
  }
...
function formatMenu(options = {}) {
  const prefix = options.compact ? 'Again? ' : 'Pick your next move:';
  return [
    `${prefix}`,
...
function formatVerdictHeader(result, options = {}) {
  ...
  return hasFindings
    ? `${color.dim('Verdict:')} ${color.red(color.bold('YOU MIGHT BE FUCKED, BRO'))} — ${color.yellow(color.bold(countLabel))}`
    : `${color.dim('Verdict:')} ${color.green(color.bold('NOT FUCKED BY THIS SNAPSHOT'))} — ${color.green(countLabel)}`;
}
