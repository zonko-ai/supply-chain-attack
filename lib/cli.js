...
function formatResult(result, options = {}) {
  ...
  lines.push('');
  lines.push(color.bold('Packages with postinstall network fetch'));
...
function formatEducation(result, options = {}) {
  ...
  if (result.suspiciousScripts && result.suspiciousScripts.length) {
    lines.push('');
    lines.push(`${color.yellow('SCRIPT')} ${color.bold('postinstall scripts with network-fetch behavior')}`);
...
function buildAgentPrompt(result) {
  ...
  if (result.suspiciousScripts && result.suspiciousScripts.length) {
    lines.push('');
    lines.push('Packages with postinstall network-fetch scripts:');
...
function campaignSummary(result, limit = 3) {
  ...
  if (result.suspiciousScripts && result.suspiciousScripts.length) campaigns.push('postinstall network-fetch heuristic');
